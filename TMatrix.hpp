...
private:
  ...
  bool _spheroidal_used{false};
  float_type _ab, _rv, _lambda;
    std::complex<float_type> _ri;

    void get_tmatrix_layer(const uint_fast32_t nmax, const uint_fast32_t mm) {
        double x = _ab, y = _rv, z = _lambda;
        boost::multiprecision::float128 ab(x), rv(y), lambda(z);
        boost::multiprecision::complex128 ri(_ri.real(), _ri.imag());
        boost::multiprecision::complex128* res = new boost::multiprecision::complex128[4*nmax*nmax];
        int32_t lnum = nmax;
        int32_t m = mm;
        if (m == 0) {
            ++lnum;
        ...
        __tmatrix_MOD_get_spheroidal_tmatrix(&ab, &rv, &lambda, &lnum, &ri, &m, res);
        uint_fast32_t start = 0;
        if (m == 0) {
            start = 1;
        }
        for (uint_fast32_t i = start; i < 2 * nmax + start; ++i) {
            for (uint_fast32_t j = start; j < 2 * nmax + start; ++j) {
                _T[0](i - start, j - start) = std::complex<float_type>(res[i * nmax + j]);
            }
        }
...
public:
  ...
  inline TMatrix(const float_type wavelength,
                 ...
                 const uint_fast32_t nmax, const uint_fast32_t ngauss, bool use_spheroidal = false)
      : _nmax(nmax), _ngauss(ngauss), _dd(nmax, float_type(0.)),
        ...
        _djkrmr(2 * ngauss, nmax), _spheroidal_used(use_spheroidal), _ab(axis_ratio), _rv(R_V),
